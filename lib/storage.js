...
const createStorage = ({ requireEnv, getEnv }) => {
  ...
  const uploadToCos = async ({ key, buffer, contentType }) => {
    ...
    if (shouldUseDev) {
      devFiles.set(key, { buffer, contentType: contentType || "application/octet-stream" });
      const publicBaseUrl = (process.env.PUBLIC_BASE_URL || "").replace(/\/+$/, "");
      if (publicBaseUrl) return `${publicBaseUrl}/__dev_files/${key}`;
      const port = process.env.PORT ? Number.parseInt(process.env.PORT, 10) : 3000;
      return `http://localhost:${Number.isFinite(port) ? port : 3000}/__dev_files/${key}`;
    }
