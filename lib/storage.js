...
const createStorage = ({ requireEnv, getEnv }) => {
  ...
  const getDb = async () => {
    ...
    await Promise.allSettled([
      ...
      db.collection("wy_guardian_relations").createIndex({ elderOpenid: 1 }),
      db.collection("wy_guardian_relations").createIndex({ guardianOpenid: 1 }),
      db.collection("wy_guardian_relations").createIndex({ guardianMpOpenid: 1 }),
      ...
      db.collection("wy_ai_companion_memories").createIndex({ updatedAt: -1 }),
    ]);
