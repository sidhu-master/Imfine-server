...
const createWeChatClient = ({ db, getEnv }) => {
  ...
  const createMpQr = async ({ scene, expireSeconds, withImage }) => {
    ...
    if (withImage === false) return { ok: true, ticket, qrUrl };

    const img = await fetchBuffer(qrUrl);
    if (!img.ok) return { ok: false, error: `qrcode download failed: ${img.status}` };
    return { ok: true, ticket, buffer: img.buffer, qrUrl };
  };
