...
const createWeChatClient = ({ db, getEnv }) => {
  ...
  const decryptMiniData = ({ sessionKey, encryptedData, iv }) => {
    const crypto = require("crypto");
    const key = sessionKey ? Buffer.from(String(sessionKey), "base64") : Buffer.from([]);
    const encrypted = encryptedData ? Buffer.from(String(encryptedData), "base64") : Buffer.from([]);
    const ivBuf = iv ? Buffer.from(String(iv), "base64") : Buffer.from([]);
    if (!key.length || !encrypted.length || !ivBuf.length) return { ok: false, error: "missing data" };
    let decoded = "";
    try {
      const decipher = crypto.createDecipheriv("aes-128-cbc", key, ivBuf);
      decipher.setAutoPadding(true);
      decoded = decipher.update(encrypted, undefined, "utf8");
    ...
    const appid = getEnv("MINIAPP_APPID");
    const watermark = data && data.watermark ? data.watermark : null;
    if (appid && watermark && watermark.appid && String(watermark.appid) !== String(appid)) {
      return { ok: false, error: "appid mismatch" };
    }
    return { ok: true, data };
  };
  ...
  const getMpUserInfo = async ({ openid, lang }) => {
    const userOpenid = String(openid || "");
    if (!userOpenid) return { ok: false, error: "missing openid" };
    const t = await getAccessToken("mp");
    if (!t.ok) return t;
    const l = lang != null ? String(lang || "") : "zh_CN";
    const url = `https://api.weixin.qq.com/cgi-bin/user/info?access_token=${encodeURIComponent(
      t.accessToken
    )}&openid=${encodeURIComponent(userOpenid)}&lang=${encodeURIComponent(l || "zh_CN")}`;
    const r = await fetchJson(url);
    if (!r.ok || !r.data || r.data.errcode) {
      return { ok: false, error: r.data ? JSON.stringify(r.data) : r.text || "user info fetch failed" };
    }
    return {
      ok: true,
      openid: userOpenid,
      unionid: r.data.unionid || "",
      subscribe: r.data.subscribe != null ? Number(r.data.subscribe) || 0 : null,
      nickname: r.data.nickname || "",
    };
  };

  const resolveUserByMpOpenid = async ({ mpOpenid, now }) => {
    const mp = String(mpOpenid || "");
    if (!mp) return { ok: false, error: "missing mpOpenid" };
    const u = await getMpUserInfo({ openid: mp });
    if (!u.ok) return u;
    const unionid = u.unionid ? String(u.unionid) : "";
    if (!unionid) return { ok: true, mpOpenid: mp, unionid: "", userOpenid: "", bound: false };

    const users = db.collection("wy_users");
    const doc = await users.findOne({ unionid }, { projection: { _id: 1, mpOpenid: 1 } });
    const userOpenid = doc && doc._id ? String(doc._id) : "";
    if (!userOpenid) return { ok: true, mpOpenid: mp, unionid, userOpenid: "", bound: false };

    const existingMpOpenid = doc && doc.mpOpenid ? String(doc.mpOpenid) : "";
    if (existingMpOpenid && existingMpOpenid === mp) {
      return { ok: true, mpOpenid: mp, unionid, userOpenid, bound: false };
    }

    const t = now instanceof Date ? now : new Date();
    await users.updateOne(
      { _id: userOpenid },
      { $set: { mpOpenid: mp, mpOpenidUpdatedAt: t, updatedAt: t } }
    );
    return { ok: true, mpOpenid: mp, unionid, userOpenid, bound: true };
  };

  const isSameUserByElderAndMpOpenid = async ({ elderOpenid, mpOpenid, now }) => {
    const elder = String(elderOpenid || "");
    const mp = String(mpOpenid || "");
    if (!elder || !mp) return { ok: false, error: "missing openid" };

    const users = db.collection("wy_users");
    const elderDoc = await users.findOne({ _id: elder }, { projection: { mpOpenid: 1, unionid: 1 } });
    const elderMpOpenid = elderDoc && elderDoc.mpOpenid ? String(elderDoc.mpOpenid) : "";
    if (elderMpOpenid && elderMpOpenid === mp) {
      return {
        ok: true,
        same: true,
        reason: "mp_openid_match",
        elderOpenid: elder,
        mpOpenid: mp,
        userOpenid: elder,
      };
    ...
    const elderUnionid = elderDoc && elderDoc.unionid ? String(elderDoc.unionid) : "";
    const resolved = await resolveUserByMpOpenid({ mpOpenid: mp, now });
    if (!resolved.ok) return resolved;

    const userOpenid = resolved.userOpenid ? String(resolved.userOpenid) : "";
    if (!elderUnionid || !resolved.unionid) {
      return {
        ok: true,
        same: false,
        reason: elderUnionid ? "mp_unionid_missing" : "elder_unionid_missing",
        elderOpenid: elder,
        mpOpenid: mp,
        userOpenid,
      };
    ...
    return {
      ok: true,
      same: Boolean(userOpenid && userOpenid === elder),
      reason: userOpenid && userOpenid === elder ? "user_index_match" : "not_match",
      elderOpenid: elder,
      mpOpenid: mp,
      userOpenid,
    };
  ...
  return {
    ...
    createWxaShortLink,
    getMpUserInfo,
    resolveUserByMpOpenid,
    isSameUserByElderAndMpOpenid,
    sendMpText,
