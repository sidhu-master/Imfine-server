...
const extractProfileUpdates = (text) => {
  ...
  if (likeMatch) {
    const value = normalizeText(likeMatch[1], 32);
    if (value && !/[?？吗么嘛]|怎样|怎么|什么/.test(value)) updates.preferences.push(`喜欢${value}`);
  }

  const dislikeMatch = s.match(/我不喜欢([^。！？!?]{1,32})/);
  if (dislikeMatch) {
    const value = normalizeText(dislikeMatch[1], 32);
    if (value && !/[?？吗么嘛]|怎样|怎么|什么/.test(value)) updates.dislikes.push(`不喜欢${value}`);
  }
